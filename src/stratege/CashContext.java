...
/**
 * 策略模式+简单工厂模式：
 * 策略模式：Context 传入父类，调用父类方法（各子类的实现），返回结果
 * 简单工厂模式：Factory 创建父类对象，返回父类对象
 * 策略模式+简单工厂模式：创建父类对象并不返回，调用父类方法，返回结果
 * 策略模式+简单工厂模式：增加算法，修改Context中的判断分支（与简单工厂模式类似）-这点要改进需利用反射，抽象工厂模式
